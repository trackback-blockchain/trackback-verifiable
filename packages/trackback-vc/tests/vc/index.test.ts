...
describe('VC Tests', () => {
    describe('Check credentials Tests', () => {
        ...
        it('validate credential @context', () => {
            expect(() => check({})).to.throw(Error, 'Credentials must have a "@context" property');
        });

        it('Credentials first item is a URI: https://www.w3.org/2018/credentials/v1', () => {
            expect(() => check({
                '@context': ['']
            })).to.throw(Error, 'Credentials first item is a URI with the value https://www.w3.org/2018/credentials/v1');
        });
    ...
    describe('VC Tests for validate ', () => {
        ...
        it('validate credential @context', () => {
            expect(() => new VC().validate({})).to.throw(Error, 'Credentials must have a "@context" property');
        });

        it('Credentials first item is a URI: https://www.w3.org/2018/credentials/v1', () => {
            expect(() => new VC().validate({
                '@context': ['']
            })).to.throw(Error, 'Credentials first item is a URI with the value https://www.w3.org/2018/credentials/v1');
        });
    ...
    describe('VC Tests for issueJWT ', () => {
        ...
        it('Credentials should be valid ', async () => {
            ...
            expect(JSON.stringify(payload.vc)).to.be.equal(JSON.stringify(credential))
