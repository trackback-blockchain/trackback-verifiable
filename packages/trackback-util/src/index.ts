...
import { blake2AsHex, mnemonicGenerate, mnemonicToMiniSecret, naclKeypairFromSeed } from '@polkadot/util-crypto';
...
export class TrackBackUtils {
  ...
  generateKeyPair() {
    ...
    // Generate new public/secret keypair from supplied seed
  ...
  base64Encode(st: string): string {
    return Buffer.from(st, "utf8").toString("base64");
  }

  base64Decode(st: string): string {
